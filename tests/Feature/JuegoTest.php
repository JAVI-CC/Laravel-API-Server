...
class JuegoTest extends TestCase
{
    ...
    public function test_obtener_juego_mediante_slug(): void
    {
        ...
        $res->assertStatus(200);
        $res->assertJson($json);
        $res->assertJsonStructure($this->getNewResourceStructureJuego());
    }
    ...
    public function test_insertar_juego(): void
    {
        ...
        $this->assertStringContainsString('dropbox', $juego->url_imagen);
        $res->assertStatus(201);
        $this->assertDatabaseCount(Juego::class, 1);
        $res->assertJson($this->getNewResourceJuego($juego));
        $res->assertJsonStructure($this->getNewResourceStructureJuego());
    ...
    public function test_actualizar_juego(): void
    {
        ...
        $this->assertStringContainsString('dropbox', $updateJuego->url_imagen);
        $res->assertStatus(200);
        $this->assertDatabaseCount(Juego::class, 1);
        $res->assertJson($this->getNewResourceJuego($updateJuego));
        $res->assertJsonStructure($this->getNewResourceStructureJuego());
